...
const SYNC = [
  {
    ...
    a: 'playbook',
    b: 'forkerRef',
    strings: ['## 1. Connect VVibe', '## 2. Register Portaly Payment', '## 3. Provision', '## 4. Make it yours', '## 5. Deploy to InsForge', '## Cheat sheet', 'insforge.dev/auth/sign-up?ref=VVIBE', 'npx @vvibe/cli connect', 'vibe_update_brand'],
  },
  {
  ...
  },
  {
    ...
    a: 'marker',
    b: 'markerRef',
    strings: ['vvibe-optimized', 'recommended host + backend', 'insforge.dev/auth/sign-up?ref=VVIBE', 'Connecting VVibe', 'npx @vvibe/cli connect', 'vibe_update_brand'],
  },
]
for (const s of SYNC)
  check(`sync: ${s.label}`, () => {
